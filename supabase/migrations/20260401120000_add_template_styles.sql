-- Create template_styles table for preset report styles
...
CREATE INDEX IF NOT EXISTS idx_template_styles_project_id
  ON template_styles(project_id);

ALTER TABLE template_styles ENABLE ROW LEVEL SECURITY;

CREATE POLICY "Users can view template styles in own projects"
...
CREATE POLICY "Users can create template styles in own projects"
...
CREATE POLICY "Users can update template styles in own projects"
...
CREATE POLICY "Users can delete template styles in own projects"
...
DROP TRIGGER IF EXISTS template_styles_updated_at ON template_styles;

CREATE TRIGGER template_styles_updated_at
  BEFORE UPDATE ON template_styles
  FOR EACH ROW EXECUTE FUNCTION public.handle_updated_at();

-- Add selected_style_id to template_fingerprints
ALTER TABLE template_fingerprints
  ADD COLUMN IF NOT EXISTS selected_style_id UUID;
...
BEGIN
  IF NOT EXISTS (
    SELECT 1
    FROM pg_constraint
    WHERE conname = 'template_fingerprints_selected_style_id_fkey'
  ) THEN
    ALTER TABLE template_fingerprints
      ADD CONSTRAINT template_fingerprints_selected_style_id_fkey
      FOREIGN KEY (selected_style_id) REFERENCES template_styles(id)
      ON DELETE SET NULL;
  END IF;
END $$;

CREATE INDEX IF NOT EXISTS idx_template_fingerprints_selected_style_id
  ON template_fingerprints(selected_style_id);

-- Seed function for template styles per project
CREATE OR REPLACE FUNCTION seed_template_styles_for_project(project_uuid UUID)
RETURNS void
LANGUAGE plpgsql
AS $$
BEGIN
  INSERT INTO template_styles (project_id, name, description, preview, styles, tables, page)
  SELECT project_uuid, s.name, s.description, s.preview, s.styles, s.tables, s.page
  FROM (
    VALUES
      (
        '经典宋体',
  ...
  WHERE NOT EXISTS (
    ...
    WHERE ts.project_id = project_uuid AND ts.name = s.name
  );
END;
$$;

-- Seed styles for existing projects
DO $$
DECLARE
  proj RECORD;
BEGIN
  FOR proj IN SELECT id FROM projects LOOP
    PERFORM seed_template_styles_for_project(proj.id);
  END LOOP;
END $$;

-- Ensure template fingerprints select default style
UPDATE template_fingerprints tf
SET selected_style_id = ts.id
FROM template_styles ts
WHERE ts.project_id = tf.project_id
  AND ts.name = '经典宋体'
  AND tf.selected_style_id IS NULL;

-- Auto seed on new projects
CREATE OR REPLACE FUNCTION handle_project_template_styles()
RETURNS trigger
LANGUAGE plpgsql
AS $$
BEGIN
  PERFORM seed_template_styles_for_project(NEW.id);
  RETURN NEW;
END;
$$;

DROP TRIGGER IF EXISTS projects_seed_template_styles ON projects;
CREATE TRIGGER projects_seed_template_styles
  AFTER INSERT ON projects
  FOR EACH ROW EXECUTE FUNCTION handle_project_template_styles();

-- Set default style on new template_fingerprints rows
...
DROP TRIGGER IF EXISTS template_fingerprints_default_style ON template_fingerprints;
CREATE TRIGGER template_fingerprints_default_style
  BEFORE INSERT ON template_fingerprints
  FOR EACH ROW EXECUTE FUNCTION set_default_template_style_on_fingerprint();
