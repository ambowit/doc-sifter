...
const log = (msg: string, data?: unknown) =>
  console.log(`[ocr-task] ${msg}${data !== undefined ? " " + JSON.stringify(data) : ""}`);
...
serve(async (req) => {
  ...
  try {
    ...
    const { fileId, fileUrl, fileName, mimeType } = reqBody;
    ...
    log("Submitting OCR task", { fileId, fileName, callbackUrl, timestamp });
    ...
    return new Response(
      JSON.stringify({ success: true, fileId, taskId, async: true }),
